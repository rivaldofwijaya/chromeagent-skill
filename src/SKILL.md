...
For project targets: A successful setup prints the absolute path it wrote; check that path against the project root. For Codex, validate the printed path against `~/.codex/config.toml`, not the project root; the output override does not change its global scope. `--out-dir` and `-OutDir` require an existing directory; neither script creates it.
